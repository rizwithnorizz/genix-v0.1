...
const DeepSeekTest = () => {
  ...
  const filteredSchedules = jsonResponse?.filter(
    (schedule) => selectedSection === '' || schedule.section_name === selectedSection
  );

  // Get all unique section names for the filter dropdown
  const sections = jsonResponse
    ? Array.from(new Set(jsonResponse.map((schedule) => schedule.section_name)))
    : [];
    ...
    <Layout>
      ...
      <div className="flex flex-wrap overflow-y-auto shadow-xl p-5 gap-1">
        {filteredSchedules?.map((json, index) => (
          ...
          >
            <div className="text-sm break-words">{json.department}</div>
            <div className="text-sm break-words">{json.section_name}</div>
            <div className="text-sm break-words">{json.subject_code}</div>
            <div className="text-sm break-words">{json.time_slot}</div>
            <div className="text-sm break-words">{json.day_slot}</div>
            <div className="text-sm break-words">{json.instructor}</div>
            <div className="text-sm break-words">{json.room_number}</div>
          </div>
        ))}
      </div>
    </Layout>
