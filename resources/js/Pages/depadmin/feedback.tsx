...
const Feedback: React.FC = () => {
    const [studentFeedbacks, setStudentFeedbacks] = useState<studentFeedback[] | null>(null);
    const [instructorFeedbacks, setInstructorFeedbacks] = useState<instructorFeedback[] | null>(null);
    ...
    const toggleFeedbackStatus = async (type: 'student' | 'instructor', id: number) => {
        try {
    ...
    // Navigation
    const [activeTab, setActiveTab] = useState<'Student Feedback' | 'Instructor Feedback'>('Student Feedback');
            ...
            <main className="col-span-3 space-y-4">
                ...
                <div className="bg-white p-4 rounded-2xl shadow-lg">
                    ...
                    <div className="flex border-b mb-4">
                        <button 
                            className={`px-4 py-2 ${activeTab === 'Student Feedback' ? 'border-b-2 border-blue-500 font-semibold' : ''}`}
                            onClick={() => handleSwitchTabs('Student Feedback')}
                        >
                            Student Feedback
                        </button>
                        <button 
                            className={`px-4 py-2 ${activeTab === 'Instructor Feedback' ? 'border-b-2 border-blue-500 font-semibold' : ''}`}
                            onClick={() => handleSwitchTabs('Instructor Feedback')}
                        >
                            Instructor Feedback
                        </button>
                    </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {studentFeedbacks?.map((feedback, idx) => (
                                            <tr key={idx}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    <button
                                                        onClick={() => toggleFeedbackStatus('student', feedback.id)}
                                                    ...
                                                    >
                                                        {feedback.status ? 'Resolved' : 'Pending'}
                                                    </button>
                                                </td>
                                            </tr>
                                        ))}
                        ...
                        <div className="bg-white p-4 rounded-2xl shadow-lg">
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {instructorFeedbacks?.map((feedback, idx) => (
                                            <tr key={idx}>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    {feedback.name}
                                                </td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    <button
                                                        onClick={() => toggleFeedbackStatus('instructor', feedback.id)}
                                                        className={`px-3 py-1 rounded-full text-xs font-medium ${
                                                            feedback.status 
                                                                ? 'bg-green-100 text-green-800 hover:bg-green-200' 
                                                                : 'bg-red-100 text-red-800 hover:bg-red-200'
